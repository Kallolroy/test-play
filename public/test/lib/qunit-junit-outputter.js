...
(function() {
	...
	cModuleTestCaseResult.getFailureXml = function(details) {
		var message = escape(details.message) || "";
	    if (details.expected) {
	        if (message) {
	            message += ", ";
	        }
	        message = "expected: " + escape(details.expected) + ", but was: " + escape(details.actual);
	    }
	...
	cModuleTestCaseResult.prototype.getXml = function(moduleName) {
		...
		var xml = ['\t\t<testcase name="', escape(name),
						'" classname="', escape(moduleName),
	...
	cModuleResult.prototype.getXml = function(id) {
		...
		// Construct the XML
		var xml = ['\t<testsuite id="', escape(id), 
					'" package="', escape(this.name), 
	...
	cJUnitOutputter.prototype.writeSystemError = function(msg) {
		this.consoleLog('<?xml version="1.0" encoding="UTF-8"?>');
		this.consoleLog('<testsuites>');
		this.consoleLog('<system-err>' + escape(msg) + '</system-err>');
		this.consoleLog('</testsuites>');
	};
